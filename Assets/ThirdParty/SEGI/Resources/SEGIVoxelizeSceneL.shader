...
// Attenuation smoothly decreases to light range.
		...
		uint GetSEGIPerObjectLightIndexOffset()
		{
#if USE_STRUCTURED_BUFFER_FOR_LIGHT_DATA
		...
		int GetSEGIPerObjectLightIndex(uint index)
		{
#if USE_STRUCTURED_BUFFER_FOR_LIGHT_DATA
			uint offset = GetSEGIPerObjectLightIndexOffset();
		...
		int GetSEGIAdditionalLightsCount()
		{
#if !defined(_ADDITIONAL_LIGHTS) && !defined(USE_FORWARD_PLUS)
			return 0;
#elif defined(USE_CLUSTER_LIGHT_LOOP)
			return 0;
#else
			return int(unity_LightData.y);
#endif
		}

		int GetSEGIAdditionalLightIndex(uint index)
		{
#if defined(USE_CLUSTER_LIGHT_LOOP)
			return index;
#else
			return GetSEGIPerObjectLightIndex(index);
#endif
		}
...
#if defined (_ADDITIONAL_LIGHTS) || defined (USE_FORWARD_PLUS)
								int additionalLightsCount = min(GetSEGIAdditionalLightsCount(), MAX_VISIBLE_LIGHTS);
								[loop] //v1.9.9.8 - Ethereal v1.1.8h
								for (uint lightLoopIndex = 0; lightLoopIndex < additionalLightsCount; lightLoopIndex++) //v1.9.9.8 - Ethereal v1.1.8h
								{
									//LIGHT 1
									float distToRayStartA = length(_WorldSpaceCameraPos - p[i].pos);//v1.9.9.8 - Ethereal v1.1.8h
									int lightIndex = GetSEGIAdditionalLightIndex(lightLoopIndex);

		//#ifdef URP11//v1.8
									Light light = GetAdditionalPerObjectLight(lightIndex, p[i].pos);
									light.shadowAttenuation = AdditionalLightShadow(lightIndex, p[i].pos, light.direction, half4(1, 1, 1, 1), half4(0, 0, 0, 0));

									//COOKIE
									//#if defined(_LIGHT_COOKIES)
									float3 cookieColor = SampleAdditionalLightCookie(lightIndex, p[i].pos);
		...
		//#endif
					...
					float4 frag(g2f input) : SV_TARGET
					{
						...
						float2 shadowTexel = SEGISunDepth_TexelSize.xy;
						float2 shadowUv = shadowPos.xy;
						float shadowBias = 0.2525;

						float sunDepth = 0.0;
						sunDepth += tex2Dlod(SEGISunDepth, float4(shadowUv + shadowTexel * float2(-0.5, -0.5), 0, 0)).x;
						sunDepth += tex2Dlod(SEGISunDepth, float4(shadowUv + shadowTexel * float2(0.5, -0.5), 0, 0)).x;
						sunDepth += tex2Dlod(SEGISunDepth, float4(shadowUv + shadowTexel * float2(-0.5, 0.5), 0, 0)).x;
						sunDepth += tex2Dlod(SEGISunDepth, float4(shadowUv + shadowTexel * float2(0.5, 0.5), 0, 0)).x;
						sunDepth *= 0.25;
						...
						float sunVisibility = smoothstep(-0.0015, 0.0015, sunDepth - shadowPos.z + shadowBias);
						...
						if (shadowlessLocalPower != 0) {
							...
							int additionalLightsCount = min(GetSEGIAdditionalLightsCount(), MAX_VISIBLE_LIGHTS);
							[loop]
							for (uint lightLoopIndex = 0; lightLoopIndex < additionalLightsCount; lightLoopIndex++)
							{
								int lightIndex = GetSEGIAdditionalLightIndex(lightLoopIndex);
								Light light = GetAdditionalPerObjectLight(lightIndex, input.worldPos.xyz);
								float3 cookieColor = SampleAdditionalLightCookie(lightIndex, input.worldPos.xyz);
								light.color *= cookieColor;
								result.rgb += color.rgb * tex.rgb * light.color * shadowlessLocalPower * light.distanceAttenuation;
							}
#else

...
SubShader
{
	...
	Pass //2 Blend with scene
	{
		CGPROGRAM
			...
			sampler2D SurfaceCacheRadiance;
			...
			float SurfaceCacheGIBlend;
			float SurfaceCacheReflectionBlend;
			float SurfaceCacheMinConfidence;
			...
			float4 surfaceCache = tex2D(SurfaceCacheRadiance, input.uv.xy);
			float4 viewSpacePositionGI = GetViewSpacePosition(coord);
			float3 positionWS = mul(CameraToWorld, viewSpacePositionGI).xyz;
			float3 worldNormalAtlas = normalize(tex2D(_CameraDepthNormalsTexture, input.uv.xy).rgb * 2.0 - 1.0);
			if (dot(worldNormalAtlas, worldNormalAtlas) < 0.1)
			{
				worldNormalAtlas = float3(0.0, 1.0, 0.0);
			}
			float4 surfaceCardAlbedo = 0.0.xxxx;
			float4 surfaceCardRadiance = 0.0.xxxx;
			float surfaceCardWeight = 0.0;
			float surfaceCacheWeight = 0.0;
			float3 giComposite = gi;
			if (UseSurfaceCache > 0)
			{
				surfaceCacheWeight = smoothstep(SurfaceCacheMinConfidence, 1.0, surfaceCache.a);
				float diffuseFallbackWeight = surfaceCacheWeight * SurfaceCacheGIBlend;
				if (DoReflections > 0)
				{
					diffuseFallbackWeight *= 0.35;
				}
				float3 cacheTarget = max(giComposite, surfaceCache.rgb);
				giComposite = lerp(giComposite, cacheTarget, saturate(diffuseFallbackWeight));
			}
			// Legacy surface-card atlas path disabled pending rewrite.
//float3 scene = tex2D(_MainTex, input.uv.xy).rgb;
				float3 reflections = tex2D(Reflections, input.uv.xy).rgb;
				
				//float3 result = scene + gi * albedoTex.a * albedoTex.rgb; //v1.2f
				float3 result = scene + albedo * giComposite * albedoTex.a * albedoTex.rgb * 10;

				if (contrastA1 >= 20) {
					giComposite = saturate(giComposite * albedoSEGIB);
					result = scene + giComposite;
				}
				else
				if (contrastA1 >= 10) {
					result = scene + (giComposite * scene * gialpha);
				}
				else
				if (contrastA1 >= 5) {
					result = scene + albedoSEGIB * giComposite * contrastA1 * scene + albedoSEGIB * giComposite * 0.01 * contrastA1;
				}
				...
				if (DoReflections > 0)
				{
					...
					//v2.5
					float4 specc = tex2D(_GBuffer1, coord);
					...
					//v2.5
					float4 spec = tex2D(_GBuffer0, coord) * ReflectControl.y * (depthA)+specc * ReflectControl.z * (depthA);
					...
					if (specc.g != 0 && specc.b != 0) {
						specularColor = spec.rgb * ReflectControl.x;
					}
					else {
						specularColor = float3(specc.r, specc.r, specc.r) * ReflectControl.x;
					}
					...
					fresnel *= saturate(smoothness * 4.0);
										...
										float3 reflectionSource = reflections;
					if (UseSurfaceCache > 0)
					{
						float reflectionLuma = dot(reflectionSource, float3(0.2126, 0.7152, 0.0722));
						float reflectionNeed = saturate(1.0 - reflectionLuma);
						float reflectionPriority = saturate(lerp(0.35, 1.0, reflectionNeed) * saturate(smoothness * 1.5));
						float3 cacheReflection = max(reflectionSource, surfaceCache.rgb * lerp(0.5, 1.0, smoothness));
						reflectionSource = lerp(reflectionSource, cacheReflection, surfaceCacheWeight * SurfaceCacheReflectionBlend * reflectionPriority);
					}
					// Legacy surface-card atlas path disabled pending rewrite.

					result = lerp(result, reflectionSource, fresnel);
				}
	...
	Pass //4 Specular/reflections trace
	{
		...
		CGPROGRAM
			...
			float4 frag(v2f input) : SV_Target
			{
				...
				float4 spec = tex2D(_GBuffer2, coord); //float4 spec = tex2D(_GBuffer0, coord); //v2.5
				//float4 spec = tex2D(_CameraGBufferTexture1, coord);
				...
				//v2.5
				float3 worldNormal = normalize(tex2D(_GBuffer2, coord).rgb) * depthA;
				...
				//v2.1.34 - COLOR - v0.7
				if (ditherControl.x != 0 && spec.a < 0.82 && dither.x > 0.001 * ditherControl.x) {
					ReflectionSteps = ReflectionSteps / ditherControl.w; //float2 dither = rand(coord + (float)FrameSwitch * 0.011734);
				}


				float smoothness = spec.a;  //float smoothness = spec.a * 0.5; //v2.5
	...
	Pass //12 Temporal blending without motion vectors (for legacy support)
	{
		...
		////// END BLEND	
				Pass //13 BLIT
			{
				...
				#include "BlitLumenLike.hlsl"//v0.2
			...
			Pass //18 == 2 Blend with scene 2
			{
				...
				float4 Frag(VaryingsB input) : SV_Target0
				{
					...
					UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
						...
						float3 scene = SAMPLE_TEXTURE2D_X_LOD(_BlitTexture, sampler_LinearRepeat, coord, _BlitMipLevel).rgb;
					...
					float4 surfaceCache = tex2D(SurfaceCacheRadiance, input.texcoord.xy);
					float4 viewSpacePositionGI = GetViewSpacePosition(coord);
					float3 positionWS = mul(CameraToWorld, viewSpacePositionGI).xyz;
					float3 worldNormalAtlas = normalize(tex2D(_CameraDepthNormalsTexture, input.texcoord.xy).rgb * 2.0 - 1.0);
					if (dot(worldNormalAtlas, worldNormalAtlas) < 0.1)
					{
						worldNormalAtlas = float3(0.0, 1.0, 0.0);
					}

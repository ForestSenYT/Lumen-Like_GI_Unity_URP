...
namespace LumenLike
{
   ...
   // [MovedFrom(true, "UnityEngine.Experimental.Rendering.Universal")]
    public class RenderObjectsPassLumenLike : ScriptableRenderPass
    {
        RenderQueueType renderQueueType;
        FilteringSettings m_FilteringSettings;
        RenderObjectsLumenLike.CustomCameraSettings m_CameraSettings;
        ...
        public RenderObjectsPassLumenLike(string profilerTag, RenderPassEvent renderPassEvent, string[] shaderTags, RenderQueueType renderQueueType, 
            int layerMask, RenderObjectsLumenLike.CustomCameraSettings cameraSettings, int isCascade)            
        {
            profilingSampler = new ProfilingSampler(profilerTag);
            Init(renderPassEvent, shaderTags, renderQueueType, layerMask, cameraSettings, isCascade);
        }

       // internal RenderObjectsPassLumenLike(URPProfileId profileId, RenderPassEvent renderPassEvent, string[] shaderTags, RenderQueueType renderQueueType, int layerMask, RenderObjectsLumenLike.CustomCameraSettings cameraSettings)
        //{
        //    profilingSampler = ProfilingSampler.Get(profileId);
       //     Init(renderPassEvent, shaderTags, renderQueueType, layerMask, cameraSettings);
        //}

        internal void Init(RenderPassEvent renderPassEvent, string[] shaderTags, RenderQueueType renderQueueType, int layerMask, RenderObjectsLumenLike.CustomCameraSettings cameraSettings, int isCascade)
        {
        ...
        {
            internal RenderObjectsLumenLike.CustomCameraSettings cameraSettings;
            ...
            ref PassData passData, ScriptableRenderContext context, RenderGraph renderGraph, bool useRenderGraph)
        {
            ...
            //v0.1 if shader not defined, grab by material if defined or by LumenLike
            drawingSettings.overrideShader = overrideShader;            
            if (Camera.main != null )
            {
                LumenLike LumenLike = Camera.main.GetComponent<LumenLike>();
                if (LumenLike != null)
                {
                    drawingSettings.overrideShader = LumenLike.preRenderers.shader;
                }
            }
              ...
              //  }
                ...
                if (Camera.main != null)
                {
                    if (isCascade == 0)
                    {
                        ...
                        if (LumenLike != null)
                        {
                            RTHandle renderTexture = RTHandles.Alloc(LumenLike.integerVolume);
                            TextureHandle textureHandle = renderGraph.ImportTexture(renderTexture);
                            builder.SetRandomAccessAttachment(textureHandle, 1, AccessFlags.ReadWrite);
                        }
                    }
                    // Legacy cascade variants removed from the supported product path.
